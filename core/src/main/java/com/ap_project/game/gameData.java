...
public class gameData implements Serializable {
    ...
    public gameData(ArrayList<block<?>> blocks, ArrayList<pig<?>> pigs, ArrayList<bird<?>> birds, int currentBirdIndex, List<Vector2> blockPositions,List<Vector2> birdPositions,
                    List<Vector2> pigPositions,ArrayList<Integer> pigHealth,float timeGap,float gameWinTimer,float gameLoseTimer,List<Vector2> blockVelocity,List<Vector2> birdVelocity,
                    List<Vector2> pigVelocity,ArrayList<Object> bodies,List<block<?>> blocksDestroyed,List<pig<?>> pigsDestroyed) {
